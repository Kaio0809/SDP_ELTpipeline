...
def tansform_raw_dataset(dataset_file_path: str=None) -> None:
    ...
    irrelevantes = ['example','tutorial']
    df = df[~df["FILE"].str.contains("|".join(irrelevantes), case=False)]
    metricas = df.columns.tolist()[1:-1]

    df['zeros'] = (df[metricas] == 0).sum(axis=1)
    ...
    # Ao final, salve as alterações em um arquivo como o nome contido em transformed_file_path
    df_limpo.to_csv(transformed_file_path, sep=";", index=False)
